...
def load_and_filter_goemotions(cache_dir, selected_emotions, num_train=0):
    ...
    train_df = train_df[train_df["label"] != -1][["text", "label"]]
    valid_df = valid_df[valid_df["label"] != -1][["text", "label"]]
    test_df = test_df[test_df["label"] != -1][["text", "label"]]
    if num_train > 0:
...
def prepare_tokenized_datasets(tokenizer, train_df, valid_df, test_df, max_length=256):
    def token(batch): return tokenizer(batch["text"], truncation=True, padding=True, max_length=max_length)
    train_dataset = Dataset.from_pandas(train_df)
    valid_dataset = Dataset.from_pandas(valid_df)
    test_dataset = Dataset.from_pandas(test_df)
    tokenized_train = train_dataset.map(token, batched=True)
    tokenized_valid = valid_dataset.map(token, batched=True)
    tokenized_test = test_dataset.map(token, batched=True)
