...
def load_and_filter_goemotions(cache_dir, selected_emotions, num_train=0):
    ...
    If num_train <= 0, use the full train set after filtering.
    """
    ...
    train_df["label"] = train_df["labels"].apply(lambda x: x[0] if x else -1)
    valid_df["label"] = valid_df["labels"].apply(lambda x: x[0] if x else -1)
    test_df["label"] = test_df["labels"].apply(lambda x: x[0] if x else -1)
    ...
    print(f"Filtered train shape: {train_df.shape}")
    print(f"Filtered valid shape: {valid_df.shape}")
    print(f"Filtered test shape: {test_df.shape}")

    if train_df.empty:
        raise ValueError("Filtered train data is empty. Check selected emotions.")

    return train_df, valid_df, test_df, selected_indices

def oversample_training_data(train_df):
    """
    Use RandomOverSampler to balance the training data classes.
    """
...
def prepare_tokenized_datasets(tokenizer, train_df, valid_df, test_df):
    """
    Tokenize the datasets using the given tokenizer.
    """
    def tokenize(batch):
        return tokenizer(batch["text"], truncation=True, padding='longest', max_length=256, return_tensors="tf")
